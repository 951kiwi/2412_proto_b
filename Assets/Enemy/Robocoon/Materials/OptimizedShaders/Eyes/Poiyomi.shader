Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Toon/6fb3466555db1c94389db9b6734a72c1"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			)
			{
				...
				if (canGlitch)
				{
					...
					#if defined(POI_VERTEX_GLITCHING_TEXTURE)
					float uvl = o.worldPos.y * (11.6 /*_VertexGlitchDensity*/) + _Time.x * (10.0 /*_VertexGlitchMapPanSpeed*/);
					float uvr = o.worldPos.y * (11.6 /*_VertexGlitchDensity*/) - _Time.x * (10.0 /*_VertexGlitchMapPanSpeed*/);
					...
					#endif
					float time = _Time.y * (1.83 /*_VertexGlitchFrequency*/);
			...
			{
				float totalPixels = float4(8,8,0,0).x * float4(8,8,0,0).y;
				float2 uvPixel = float2((floor((1 - uv.y) * float4(8,8,0,0).y)) / float4(8,8,0,0).y, (floor(uv.x * float4(8,8,0,0).x)) / float4(8,8,0,0).x);
				float currentPixelNumber = float4(8,8,0,0).x * (float4(8,8,0,0).y * uvPixel.x) + float4(8,8,0,0).y * uvPixel.y;
				float currentPixelAlpha = currentPixelNumber / totalPixels;
				half electronBeamAlpha = frac(_Time.y * (24.0 /*_VideoCRTRefreshRate*/));
				...
				float CRTFrameTime = 1 / (24.0 /*_VideoCRTRefreshRate*/);
			...
			void applyVideoEffectsMainTex(inout float4 mainTexture, in PoiMesh poiMesh)
			{
				...
				if((0.0 /*_VideoPixelateToResolution*/))
				{
					...
					uvs = sharpSample(float4(1/float4(8,8,0,0).xy, float4(8,8,0,0).xy), uvs);
			...
			{
				#if defined(PROP_VIDEOPIXELTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float3 pixels = tex2D(_VideoPixelTexture, poiUV(poiMesh.uv[(0.0 /*_VideoPixelTextureUV*/)], float4(128,128,0,0)) * float4(8,8,0,0));
				#else
				float3 pixels = 1;
				#endif
				float2 uvs = poiMesh.uv[(0.0 /*_MainTexUV*/)];
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				clip(poiFragData.alpha - (0.3 /*_Cutoff*/));
